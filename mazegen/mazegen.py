...
class MazeGenerator:
    ...
    GENERATION_ALGORITHMS = {"dfs", "kruskal", "prim"}
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.algorithm not in self.GENERATION_ALGORITHMS:
            allowed = ", ".join(sorted(self.GENERATION_ALGORITHMS))
            raise ValueError(
                f"Unknown algorithm '{self.algorithm}'. Allowed: {allowed}"
            )
    ...
    def _generate_perfect_maze_prim(
        self,
        start_row: int,
        start_col: int,
    ) -> None:
        ...
        def add_frontiers(row: int, col: int) -> None:
            for direction in ["N", "E", "S", "W"]:
                dr, dc = self.DIRECTIONS[direction]
                nr, nc = row + dr, col + dc
                if (
                    self._is_valid_cell(nr, nc)
                    and (nr, nc) not in self.locked_cells
        ...
        add_frontiers(start_row, start_col)

        while frontier:
            idx = random.randrange(len(frontier))
            row, col, direction = frontier.pop(idx)
            dr, dc = self.DIRECTIONS[direction]
            nr, nc = row + dr, col + dc

            if (nr, nc) in visited or (nr, nc) in self.locked_cells:
                continue
    ...
    def _generate_perfect_maze_kruskal(self) -> None:
            ...
            for col in range(self.width):
                ...
                for direction in ("E", "S"):
                    ...
                    if (
                        ...
                        edges.append((row, col, direction))
        ...
        for row, col, direction in edges:
            ...
            if (row, col) not in parent or (nr, nc) not in parent:
                continue

            if find((row, col)) != find((nr, nc)):
                self._carve_passage(row, col, direction)
                union((row, col), (nr, nc))
    ...
    def generate(self) -> bool:
        ...
        if self.algorithm == "dfs":
            ...
            self._generate_perfect_maze_prim(entry_row, entry_col)
        elif self.algorithm == "kruskal":
            self._generate_perfect_maze_kruskal()
        else:
            allowed = ", ".join(sorted(self.GENERATION_ALGORITHMS))
            raise ValueError(
                f"Unknown algorithm '{self.algorithm}'. Allowed: {allowed}"
